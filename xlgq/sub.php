<?php
	...
	require_once SCRIPT_ROOT.'wx/include/common.php';
	...
	/*
	function test_msg($openid){
		$msg = urldecode(trim($_REQUEST['msg']));
		$code = _get_fuck_off_code();
		$msg = _decode_msg($msg,$code);
		$msg = @json_decode($msg,true);
	
		if (is_array($msg)){ //判断 $msg 是否能成功解密来决定是否合法
			$data['openid'] = $openid;
			...
			insertorupdate($openid,$data['score']);
			
			$ret = json_encode(array('code'=>1,'msg'=>'ok'));
			exit;
		}else{
			@error_log('bad_decode openid:['.$openid.'],code:['.$code.'],msg:['.$_REQUEST['msg'].']');
			$ret = json_encode(array('code'=>-1,'msg'=>'bad'));
			exit;
		}
	
		echo "window._send_msg_cb({$ret});";
	}
	*/
